    ...
    <header class="neo-page-header">
      <div class="capsule-search">
        <div class="search-inputs">
          ...
          <div class="divider"></div>
          <a-select v-model:value="searchForm.country" placeholder="地区" :bordered="false" class="status-select" allowClear @change="handleSearch">
            <a-select-option v-for="dict in countryDict" :key="dict.dictValue" :value="dict.dictValue">
              {{ dict.dictLabel }}
            </a-select-option>
          </a-select>
        </div>
      ...
      >
          ...
          <div class="data-empty-container">
            <div class="modern-empty-card">
              ...
              <div class="modern-empty-content">
                <h3 class="modern-empty-title">暂无色情网站数据</h3>
                <p class="modern-empty-desc">点击上方按钮添加记录</p>
              </div>
            </div>
          </div>
        </template>
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'country'">
            <span class="country-tag">{{ countryLabel(record.country) }}</span>
          </template>
...
import { getWebsitePornPage, syncWebsitePorn } from '@/api/website.ts'
import { getDictDataByDictCodeEnable } from '@/api/dict.ts'
import type { WebsitePornVo, DictData } from '@/types'
...
const columns = [
  ...
  { title: '地区', dataIndex: 'country', key: 'country', width: 90 },
...
const tableData = ref<WebsitePornVo[]>([])
const previewImg = ref<string | null>(null)
const webImgLoadedCache = reactive(_webImgCache)
const categoryDict = ref<DictData[]>([])
const countryDict = ref<DictData[]>([])

const searchForm = reactive({ name: '', category: undefined as string | undefined, country: undefined as string | undefined, pageNum: 1, pageSize: 10 })
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await getWebsitePornPage(searchForm)
...
const fetchCategoryDict = async () => {
  try {
    const res = await getDictDataByDictCodeEnable('website_pron_category')
    categoryDict.value = res.data || []
  } catch (error) {
    console.warn('获取 website_pron_category 字典失败', error)
  }
}
...
const fetchCountryDict = async () => {
  try {
    ...
    console.warn('获取 country 字典失败', error)
  }
}

const countryLabel = (code: string) => {
  const found = countryDict.value.find(d => d.dictValue === code)
  return found?.dictLabel || code || '—'
}
...
const handleSync = async () => {
  syncing.value = true
  try {
    const res = await syncWebsitePorn()
...
onMounted(() => { fetchData(); fetchCategoryDict(); fetchCountryDict() })
</script>
...
:global(.dark) .country-tag {
  background: color-mix(in srgb, #30D158 15%, transparent);
  color: #30D158;
}
