<template>
  ...
  <header class="page-header">
    <h1 class="page-title">网站推荐</h1>
    <p class="page-sub">发现有用的工具与资源</p>
  </header>
  ...
  <!-- ================= 筛选栏 ================= -->
  <div class="filter-bar" v-if="categoryDict.length">
    <!-- 分类 -->
    ...
    <!-- 地区（仅限制模式） -->
    <div class="filter-row" v-if="!appStore.showRestricted && countryDict.length">
      <span class="filter-label">地区</span>
      <div class="filter-pills">
        <button class="filter-pill" :class="{ active: selectedCountry === '' }" @click="selectedCountry = ''">全部地区</button>
        <button
          v-for="c in countryDict" :key="c.dictValue"
          class="filter-pill"
          :class="{ active: selectedCountry === c.dictValue }"
          @click="selectedCountry = c.dictValue"
        >{{ c.dictLabel }}</button>
      </div>
    </div>
  </div>
  ...
  <section class="website-grid-section">
    ...
    <div v-else-if="filteredWebsites.length" class="website-grid">
      ...
      >
        ...
        <div class="card-icon-wrap">
          <img
            v-if="site.icon && isImageUrl(site.icon)"
            :src="site.icon"
            :alt="site.name"
            class="card-icon-img"
            loading="lazy"
          />
          <span v-else class="card-icon-emoji">{{ site.icon || '🌐' }}</span>
        </div>
        ...
        <!-- 分类+地区标签 -->
        <div class="card-tags" v-if="site.category || (!appStore.showRestricted && (site as any).country)">
          <span class="card-category" v-if="site.category">{{ site.category }}</span>
          <span class="card-country" v-if="!appStore.showRestricted && (site as any).country">{{ countryLabel((site as any).country) }}
          </span>
        </div>

        <!-- 预览截图 -->
        <div class="card-preview" v-if="site.webImg" @click.prevent.stop="previewImg = site.webImg">
          <img :src="site.webImg" :alt="site.name" loading="lazy" decoding="async" />
        ...
        <!-- 描述 -->
        <p class="card-desc" v-if="site.description">{{ site.description }}</p>
      </a>
    </div>
    ...
    <div v-else class="modern-empty-card">
      ...
      <div class="modern-empty-content">
        <h3 class="modern-empty-title">暂无推荐网站</h3>
        <p class="modern-empty-desc">暂时没有可显示的网站数据。</p>
      </div>
    </div>
  </section>
...
import { getWebsitePage, getWebsitePornPage } from '@/api/website.ts'
...
const countryDict = ref<DictData[]>([])
...
const filteredWebsites = computed(() => {
  ...
  if (searchKeyword.value) {
    ...
    list = list.filter(site =>
      ...
      (site.description && site.description.toLowerCase().includes(kw))
    )
  }
  return list
})
...
const fetchWebsites = async () => {
  loading.value = true
  try {
    if (appStore.showRestricted) {
      const res = await getWebsitePage({ pageNum: pageNum.value, pageSize, name: searchKeyword.value || undefined, category: selectedCategory.value || undefined })
      websites.value = res.data.list || []
      total.value = res.data.total || 0
    } else {
      const res = await getWebsitePornPage({ pageNum: pageNum.value, pageSize, name: searchKeyword.value || undefined, category: selectedCategory.value || undefined, country: selectedCountry.value || undefined })
      websites.value = res.data.list || []
      total.value = res.data.total || 0
    }
  } catch {
...
const fetchCategoryDict = async () => {
  try {
    const dictCode = appStore.showRestricted ? 'website_category' : 'website_pron_category'
    const res = await getDictDataByDictCodeEnable(dictCode)
...
const fetchCountryDict = async () => {
  try {
    const res = await getDictDataByDictCodeEnable('country')
    countryDict.value = res.data || []
  } catch {
    countryDict.value = []
  }
}
...
const countryLabel = (value: string) => {
  const item = countryDict.value.find(d => d.dictValue === value)
